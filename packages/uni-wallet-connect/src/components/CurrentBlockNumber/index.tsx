...
export const StyledPollingDot = ({ children }: { children: ReactNode }) => {
  return (
    <div
      className="relative w-2 h-2 bg-green-500 rounded"
    >
      {children}
    </div>
  )
    ...
    <ExternalLink href={chainId && blockNumber ? getEtherscanLink(chainId, blockNumber.toString(), 'block') : ''}>
      <StyledPolling>
        <div className='flex flex-row items-center justify-center'>
          <p className="mr-2 text-sm font-medium text-green-500" style={{ opacity: isMounted ? '0.3' : '0.6' }}>
            {blockNumber}
          </p>
