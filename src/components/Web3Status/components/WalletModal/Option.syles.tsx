  ...
  return (
    <button
      className={clsx(
        ...
        clickable && 'cursor-pointer hover:border hover:border-solid hover:border-pink-500',
        disabled && 'opacity-50',
        'w-full border border-solid rounded-lg outline-none focus:shadow-md hover:shadow-md',
...
export const HeaderText = ({ children }: { children: ReactNode }) => {
  return <div className="flex flex-row font-medium text-black font-base">{children}</div>
}
...
export const IconWrapper = ({ children }: { children: ReactNode }) => {
  return <div className='flex flex-col items-end justify-center md:items-center'> {children}</div>
}
